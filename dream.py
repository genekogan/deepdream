...
def run_deepdream2(deepdream, config, img0=None):
#(deepdream, t_grad, img0, iter_n=10, step=1.5, octave_n=4, octave_scale=1.4):
    # split the image into a number of octaves
    
    t_obj = [deepdream.T(obj['layer'])[:,:,:,obj['channel']] for obj in config['objective']]
    t_score = tf.reduce_mean(t_obj) # defining the optimization objective
    t_grad = tf.gradients(t_score, deepdream.t_input)[0] # behold the power of 
    
    iter_n = config['iterations'][0]
    step = config['step']
    octave_n = config['octaves']
    octave_scale = config['octave_ratio']
    lap_n = config['lap_n']

    lap_norm_func = deepdream.laplacian_normalization(lap_n)
    ...
    for i in range(octave_n-1):
        print("octa", i)
        hw = img.shape[:2]
        lo = deepdream.resize(img, np.int32(np.float32(hw)/octave_scale))
        hi = img-deepdream.resize(lo, hw)
        img = lo
        octaves.append(hi)
    tile_size = 512
    ...
    for octave in range(octave_n):
        print('go for octave', octave)
        if octave>0:
            hi = octaves[-octave]
            img = deepdream.resize(img, hi.shape[:2])+hi
        print('img size is ', np.array(img).shape)
        print('step', step)
        for i in range(iter_n):
            #g = calc_grad_tiled(img, t_grad)
            g = deepdream.calc_grad_tiled(img, t_grad, tile_size)
            #g = lap_norm_func(g)
...
# def render_deepdream(deepdream, t_grad, img0, iter_n=10, step=1.5, octave_n=4, octave_scale=1.4):
#     # split the image into a number of octaves
#     img = img0
#     octaves = []
#     for i in range(octave_n-1):
...
# last_grad = None
# last_channel = None
# def render(deepdream, img, layer='mixed4d_3x3_bottleneck_pre_relu', channel=139, iter_n=10, step=1.5, octave_n=4, octave_scale=1.4):
#     global last_layer, last_grad, last_channel
#     if last_layer == layer and last_channel == channel:
#         t_grad = last_grad
#     else:
#         if channel == 4242:
#             t_obj = tf.square(deepdream.T(layer))
#         else:
#             t_obj = deepdream.T(layer)[:,:,:,channel]
#         t_score = tf.reduce_mean(t_obj) # defining the optimization objective
#         t_grad = tf.gradients(t_score, deepdream.t_input)[0] # behold the power of automatic differentiation!
#         last_layer = layer
#         last_grad = t_grad
#         last_channel = channel
#     img0 = np.float32(img)
#     return render_deepdream(deepdream, t_grad, img0, iter_n, step, octave_n, octave_scale)
